...
@moment.handle()
async def handle_moment(event: MessageEvent, matcher: Matcher, args: Message = CommandArg()):
    ...
    if not content:
        await matcher.send("请输入内容 和 背景ID（可选）：\n"
                          "/moment 你好，世界)\n"
                          "/moment 你好，世界 105 - 使用105号背景（105位置为bgid）\n")
        return
    if len(content) > 1200:
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            response = Apirequests.moment(watchid, bindnumber, chipid, model, content_text, bgid)
            if response['code'] == '000001':
                ...
                await matcher.send(f"好友圈动态发送失败！\n"
                                    f"----------------------\n"
                                    f"返回码：{response['code']}\n"
                                    f"Info：{response['desc']}")
