...
@name.handle()
async def name_start(event: MessageEvent, matcher: Matcher, state: T_State, args: Message = CommandArg()):
    ...
    if not content:
        await matcher.send("请发送要修改的新名称，例如：/name Zxi2233")
        return
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            response = Apirequests.name(watchid, bindnumber, chipid, model, content)
            
            if response.get('code') == '000001':
                await matcher.send(f"更改名称成功！\n"
                                    f"----------------------\n"
                                    f"New Name：{content}\n"
                                    f"----------------------\n"
                                    f"返回码：{response['code']}\n"
