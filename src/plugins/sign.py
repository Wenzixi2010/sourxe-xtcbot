...
@sign.handle()
async def sign_start(event: MessageEvent, matcher: Matcher, state: T_State, args: Message = CommandArg()):
    ...
    if not content:
        await matcher.send("请发送要修改的新个性签名，例如：/sign A Furry")
        return
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            response = Apirequests.sign(watchid, bindnumber, chipid, model, content)
            
            if response.get('code') == '000001':
                await matcher.send(f"更改个性签名成功！\n"
                                    f"----------------------\n"
                                    f"New sign：{content}\n"
                                    f"----------------------\n"
                                    f"返回码：{response['code']}\n"
...
__plugin_sign__ = "sign"
__plugin_usage__ = """
发送好友圈动态插件

命令列表：
/sign [sign] - 直接发送指定内容的动态
例如：/sign 222
"""