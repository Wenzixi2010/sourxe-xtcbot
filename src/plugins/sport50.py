...
@sport50.handle()
async def handle_sport50(event: MessageEvent, matcher: Matcher, args: Message = CommandArg()):
    ...
    if not content:
        await matcher.send("请输入时间：\n"
                          "/sport50 second - 指定时间（秒）\n")
        return
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if response['code'] == '000001':
                await matcher.send(f"Sport50米时间修改成功！\n"
                                    f"----------------------\n"
                                    f"返回码：{response['code']}\n"
                ...
                await matcher.send(f"Sport50米时间修改失败！\n"
                                    f"----------------------\n"
                                    f"返回码：{response['code']}\n"
                                    f"Info：{response['desc']}")
...
__plugin_name__ = "sport50"
__plugin_usage__ = """
Sport50米时间修改插件

命令列表：
/sport50 [Second] - Sport50米时间修改
例如：/sport50 6
"""