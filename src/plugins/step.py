...
@step.handle()
async def step_start(event: MessageEvent, matcher: Matcher, state: T_State, args: Message = CommandArg()):
    ...
    try:
        if not content:
            await matcher.send("请输入要修改步数数量，例如：/step 98799")
            return
        
        if int(content) > 98799:
            await matcher.send("步数过多（最多98799）")
            return
        elif int(content) < 0:
            await matcher.send("步数不可小于0")
            return
    except ValueError:
        await matcher.finish("指令错误！请输入有效的数字")
        return
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            response = Apirequests.step(watchid, bindnumber, chipid, model, content)
            
            if response.get('code') == '000001':
                await matcher.send(f"修改步数成功！\n"
                                   f"在自己的手表上可能看不到，但是别人能看到你修改的步数\n"
                                    f"----------------------\n"
                                    f"New step：{content}\n"
                                    f"----------------------\n"
                                    f"返回码：{response['code']}\n"
